...
<div class="card">
    <div class="card-header text-center red-dark">
        <h3 class="card-title text-center">Editar noticia</h3>
    </div>
    <div class="card-body">
        <form id="form" method="post">
            ...
            <div class="mb-3 text-center">
                ...
                <label for="file" class="form-label">Seleccione otra imagen si desea cambiar la actual:</label>
                <input type="file" class="form-control" name="image" id="image" placeholder="Imagen" aria-describedby="fileHelpId" accept="image/*">
            </div>
            <div class="mb-3">
            ...
            <input class="btn btn-success" type="submit" value="Editar noticia">
        </form>
    </div>
</div>
<script src="../JS/quill.js"></script>
<!-- Script para Quill editor -->
<script>
    var toolbarOptions = [
...
<script>
    ...
    function getNews(categories){
        //Llenado de input con la info de la noticia
        $.ajax({
            type: "POST",
            url: "../Business/newsaction.php",
            data: {
                'action': 'getSpecificNews',
                'id': $('#id').val()
            },
            dataType: "json",
            success: function(response) {
                options = "";
                // $("#image-container").html("<img class='mt-3' style='width:100%' src='data:"+response['img_type']+";base64,"+response['image']+"'>");
                $("#image-container").html("<img class='mt-3' style='width:100%' src='../Resources/News-resources/"+response['image']+"'>");
                $("#title").val(response['title']);    
                $("#bajadilla").val(response['bajadilla']);    
                ...
                for(let j = 0; j < categories.length; j++){
                    if(categories[j]['id'] == response['id_category']){
                        options += "<option selected value='"+categories[j]['id']+"'>"+categories[j]['name']+"</option>"
                    }else{
                        options += "<option value='"+categories[j]['id']+"'>"+categories[j]['name']+"</option>"
                    }
                }
    ...
    $('#form').on('submit', function (e) {
        ...
        if ( $('#title').val() == '' || $('#bajadilla').val() == '' || $('#id_category').val() == 'Seleccionar' || $('#description').val() == '') {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response['message'] == 'success') {
                        swal.fire("Registro exitoso", "Noticia editada con éxito", "success");
                        $("#form")[0].reset();
                        // quill.deleteText();
                        getAllFiles();
                    } else if (response['message'] == 'error') {
                        swal.fire("Error!", "Ha ocurrido un error al editar la noticia", "error");
...
<script>
    ...
    function saveFile(){    
        ...
        formData.append('action', 'saveFromEdit');
        formData.append('newsId', $('#id').val());
